...
public class ZulrahData
{
	...
	public Optional<StandLocation> getCurrentDynamicStandLocation()
	{
		...
		else if (current.getZulrahNpc().getType() == ZulrahType.MELEE)
		{
			...
			{
				case NORTHEAST_TOP:
					return OzoneZulrahPlugin.isFlipStandLocation() ? Optional.of(StandLocation.NORTHEAST_BOTTOM) : Optional.of(current.getAttributes().getStandLocation());
				case WEST:
					return OzoneZulrahPlugin.isFlipStandLocation() ? Optional.of(StandLocation.NORTHWEST_BOTTOM) : Optional.of(current.getAttributes().getStandLocation());
				default:
	...
	{
		if (OzoneZulrahPlugin.isZulrahReset())
		{
		...
		else if (current != null && current.getAttributes().getPrayer() != null)
		{
			...
			return isJad() ? (OzoneZulrahPlugin.isFlipPhasePrayer() ? Optional.of(invertedPhasePrayer) : Optional.of(phasePrayer)) : Optional.of(phasePrayer);
		}
		else
		{
			return Optional.empty();
